...
> **艦隊任務 ID**：`T-RG-D06`
> **狀態**：設計階段，待 PPLX 審查
> **約束**：嚴格遵守 per-project state_dir 隔離（REMAGRAPH_STATE_DIR 必須對應 project）、project_id 強制欄位、所有 store/search 必須經過安全閥門。不得依賴 default DB 作為生產使用。與 D02、D01 對齊；同時考量其他依賴 RemaGraph 的外部專案在治理層面的隔離需求。
...
- 曾發生外部專案的記憶混在 default DB 的情況，違反隔離設計

目標：自動化、安全、符合設計（project isolation + safety valve）

## 2. 設計原則（來自先前共識與 DESIGN.md）

- **安全閥門（Safety Valve）**：任何不合規輸入（未設 REMAGRAPH_STATE_DIR、project_id=default 但屬於其他外部專案、無效 state_dir）立即 reject + audit。
- **Per-project 隔離**：每個外部專案必須用各自專屬的 state 目錄（例如 `~/.local/state/remagraph-<project_id>/`），RemaGraph 核心用 default 或對應。
...
def safety_validate_project(project_id: str, *, require_env_match: bool = True):
    ...
    if project_id != "default" and configured == resolve_project_state_dir("default"):
        raise SafetyValveError(f"非 default 專案（{project_id}）不得使用 default DB")
...
- 所有外部專案的啟動點（例如：專案自身的初始化邏輯、常駐行程啟動、測試 fixture）**強制**：
  1. 呼叫 `_ensure_remagraph_project("other-project")`
  2. 設定 `REMAGRAPH_STATE_DIR`
  3. 立即呼叫 `safety_validate_project("other-project")`
  4. 若失敗 → 直接中止啟動 + audit
...
- **不得將 default DB 視為任何外部專案的 authoritative source**。
- 遷移設計為**一次性、顯式操作**：
  - 只允許透過明確工具觸發：
    - MCP: `remagraph_migrate_project --from default --to other-project`
    - CLI: `remagraph migrate-project --from default --to other-project`
  - 內部流程：
    1. safety_validate_project("other-project")
    2. 掃描 default DB 中屬於 other-project 的記錄（依 tags / task_id / agent_id 模式，或 governance 標記）。
    3. INSERT 到 other-project 專用 DB（帶正確 project_id="other-project"）。
    4. 在 default DB 標記 `status="invalidated"`，learnings 附加 `"migrated_to: remagraph-other-project at <ts>"`。
    5. 在 default DB 的 metadata / _meta 寫入 `other_project_migrated_at` flag。
- 之後任何啟動/維護：
  - 若偵測到 flag，**不再自動掃描 default DB**。
  - 安全閥門永久阻擋 other-project 對 default DB 的 store/search/maintain。
- 舊 default DB 僅作為 archive，永不作為 other-project 生產來源。
...
- CLI: `remagraph maintain --project other-project --force`
...
## 6. 驗收條件
- 啟動任一外部專案 → 自動用獨立 DB，default 無新增外部專案資料。
