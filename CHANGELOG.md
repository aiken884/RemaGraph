...
#### Added
- **`remagraph prompt-hook` — automatic memory recall for Claude Code (UserPromptSubmit)**. Reads the hook JSON from stdin, derives the current project from the git repo (worktree-safe, same slug rules as the post-commit hook, probing both the raw directory name and its slug), opens the project's memory DB strictly read-only (`mode=ro` + `query_only` — it can never create a database, run a migration, or trigger maintenance), runs a recall-oriented FTS5 OR query built from the prompt's long tokens, and prints the top matches as an `additionalContext` block. Hard guarantees: silent zero output + exit 0 on no match, underivable project, missing DB, bad hook configuration (extra CLI args are intercepted before argparse — exit 2 would block the user's prompt), and any internal error; memory content is neutralized (`<` → `&lt;`) so a stored commit subject can never break out of the context wrapper. Cold-start latency ~0.2s. Wire it up with: `{"hooks": {"UserPromptSubmit": [{"hooks": [{"type": "command", "command": "remagraph prompt-hook"}]}]}}`.
- **CLI conventional state-dir auto-resolution**: `search`/`store`/`status` with an explicit `--project` and no `REMAGRAPH_STATE_DIR` now automatically adopt `~/.local/state/remagraph-<project>` when it exists (with the authoritative project id taken from its `project.json`), matching the v2 post-commit hook — scripts and hooks no longer need to export the env var themselves.
- **`--status all` escape hatch** for `search` (CLI and MCP): the FTS path previously returned superseded/invalidated records when `status` was unset; it now defaults to `active` everywhere (consistent with list mode), and `all` explicitly searches the full history across every query path, including `--cross-project-label`.
- **`remagraph --version`**, and a top-level `--help` that actually works (see Fixed).

#### Fixed
- **CLI dispatch**: `remagraph --help`/`-h` (and any unknown/typo'd subcommand, `--version`, or leading global flags) previously fell through to the MCP-serve fallback — printing a misleading project-binding error, or silently starting a stdio server that hung the terminal when `REMAGRAPH_PROJECT` happened to be set. The serve fallback is now limited to the two historical shapes (bare `remagraph`, leading `--project` with no subcommand); everything else goes to argparse. `serve --project` with a missing/empty value now fails loudly instead of silently falling back to the env var.
- **Search**: multi-short-token queries (e.g. CJK two-character words split by spaces — `"資料 搜尋"`) always returned zero results — the trigram-length gate measured total length while each token was matched as its own phrase; the gate now works per token, drops too-short tokens with a warning, and quoted FTS5 reserved words (`"or"`) no longer sneak past the gate via their quote characters. Cross-project fan-out now re-sorts merged results before truncation (foreign projects' better matches were systematically dropped), survives corrupt candidate DBs (`DatabaseError`, not just `OperationalError`), labels each row's origin by the row's own `project_id`, and no longer counts unreachable candidates as "searched". Empty-query `include_related` lists recent memories instead of returning nothing.
- **Store/migrate**: cross-project migration no longer silently drops records on memory-id collision (per-DB daily sequences collide whenever both projects stored on the same day — the record is re-numbered in the target instead), carries `memory_labels` along (previously lost forever: the source copy was invalidated out of label search and the target copy had no labels), and is retry-idempotent (a retry after a partial failure recognizes already-migrated records instead of duplicating the whole batch). `process_store` opens its transaction with `BEGIN IMMEDIATE` (deferred BEGIN let concurrent writers collide on the same generated id) inside the try block, and its ROLLBACK is guarded so a disk-full error is reported as itself rather than as "cannot rollback".
- **Schema/compat**: freshly-created databases seeded `min_writer_version=6` while the migration path deliberately keeps 5 — pinned v5 consumers were wrongly degraded to read-only against new DBs; both paths now agree (existing mis-seeded DBs are backfilled on next connect). v1→v4-era migrations now rebuild the FTS index after the table rebuild (row-id shift silently corrupted full-text search for any database that had ever deleted a row; v1/v2-origin databases previously ended up with a permanently empty FTS index).
- **Safety valve**: the restricted-prefix check compared directory basenames case-sensitively — on macOS's case-insensitive APFS, `REMAGRAPH` was physically the default state dir but passed the check (verified bypass); detection is now case-insensitive plus a `samefile` check that also catches symlink aliases. Validation-failure paths (including serve startup diagnostics) no longer upsert the shared project registry before validation passes — an inherited-env violation could previously overwrite a project's correct registry mapping and point cross-project fan-out at the wrong database.
- **CLI/UX hardening**: `remagraph init` rejects project names that would produce broken `project.json`/`env.sh` (previously `init --project 'x"$(cmd)'` exited 0 with a corrupted shell file), and its rules now match the model layer exactly (which now permits underscore-prefixed project ids — a common directory convention that previously made every hook write-back fail). Safety-valve rejections and store-request validation errors print clean guidance instead of raw tracebacks. The bare-environment path (no `REMAGRAPH_*` set) — which no test had ever exercised — is now covered.
- **post-commit hook (fields-schema-version 2)**: resolves the project's conventional state dir automatically (bare environments previously failed on every commit with "REMAGRAPH_STATE_DIR is not set"; an env inherited from a different project previously caused a metadata-mismatch rejection), takes the authoritative project id from `project.json` (mixed-case repo names failed on macOS and split into two databases on Linux), keeps a user's matching custom env instead of overriding it, survives environments without `HOME`, and preserves the `-commit-<hash>` suffix on long repo names (previously all commits of a long-named repo shared one truncated task_id, so each commit silently superseded the previous one's memory). The installer refuses to downgrade a newer managed hook.
- **Server**: `remagraph_maintain` now enforces the same project-binding check as the other MCP tools; the in-memory rate limiter no longer grows without bound across distinct agent ids; audit writers serialize fully before writing (concurrent appends can no longer interleave partial lines) — all `Fixed` items verified by the adversarial-review/acceptance-scan cycle described above.
